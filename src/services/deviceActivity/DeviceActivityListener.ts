...
class DeviceActivityListenerService {
  ...
  private handleAppBlockedEvent(event: AppBlockedEvent): void {
    ...
    try {
      // App blocking is now handled by foreground detection and bottom sheet overlay
      console.log('📱 [DEVICE_ACTIVITY] App launch blocked - handled by shield and foreground detection');

      // Clear the pending event from shared storage
      this.clearPendingEvent('pendingBlockingEvent');

      console.log('✅ App blocking handled for:', event.bundleIdentifier);
    } catch (error) {
