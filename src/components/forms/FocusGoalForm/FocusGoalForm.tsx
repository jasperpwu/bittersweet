  ...
  useEffect(() => {
    if (editingGoal) {
      ...
      if (goalTagIds.length === 1) {
        ...
        autoGeneratedName = `${editingGoal.period.charAt(0).toUpperCase() + editingGoal.period.slice(1)} ${tagDisplayName} Goal`;
      } else {
  ...
  const handleSubmit = () => {
    ...
    if (!goalName) {
      if (selectedTagIds.length === 1) {
        ...
        const tagDisplayName = tag?.name || 'Focus';
        goalName = `${period.charAt(0).toUpperCase() + period.slice(1)} ${tagDisplayName} Goal`;
      } else {
