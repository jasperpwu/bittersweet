...
export const FocusGoalForm: FC<FocusGoalFormProps> = ({
  ...
  useEffect(() => {
    if (editingGoal) {
      ...
      setSelectedTagNames(editingGoal.tagNames || []);
      ...
      if (editingGoal.tagNames && editingGoal.tagNames.length === 1) {
        const tagName = editingGoal.tagNames[0] || 'Focus';
  ...
  const handleSubmit = () => {
    ...
    if (!goalName) {
      if (selectedTagNames.length === 1) {
        ...
        const tagName = selectedTagNames[0] || 'Focus';
  ...
  const isValid = targetHours > 0 && selectedTagNames.length > 0;
  ...
  return (
    <ScrollView className="flex-1">
      <View className="space-y-4">
        ...
        <View>
          ...
          {showAddTag && (
            <View className="mt-3">
              ...
              <View className="flex-row flex-wrap">
                {availableTags.filter(tag => !selectedTagNames.includes(tag.name)).map((tag) => (
                  <Pressable
                    key={tag.name}
                    onPress={() => {
                      toggleTag(tag.name);
        ...
        {(selectedTagNames.length > 1 || customGoalName) && (
          <View>
            <Typography variant="subtitle-16" color="white" className="mb-2">
              Goal Name (Optional)
            </Typography>
            ...
            <TextInput
              ...
              placeholder={
                selectedTagNames.length === 1
                  ? `${period.charAt(0).toUpperCase() + period.slice(1)} ${selectedTagNames[0]} Goal`
